...
export default function WarpText({
  ...
  useEffect(() => {
    ...
    if (contextRef.current) {
      const uniforms = contextRef.current.program.uniforms;
      uniforms.uWarpStrength.value = warpStrength;
      uniforms.uWarpScale.value = warpScale;
      uniforms.uSpeed.value = speed;
      uniforms.uPointerInfluence.value = pointerInfluence;
      uniforms.uPointerStrength.value = pointerStrength;
      uniforms.uRefraction.value = refraction;
      uniforms.uRipple.value = ripple ? 1 : 0;
  ...
  useEffect(() => {
    ...
    let resizeObserver: any, intersectionObserver: any, raf = 0, disposed = false;
    let visible = true, pageVisible = !document.hidden, rasterVersion = 0;
    ...
    const loop = (now: number) => {
      ...
      pointer.active += ((pointer.activeTarget > 0 ? 1 : 0.15) - pointer.active) * 0.06;
    ...
    canvas.addEventListener('pointerleave', () => { pointer.activeTarget = 0; });
