...
import { 
  ...
  BookOpen, Home, Cpu, Workflow, Radio, Terminal,
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-40 w-full border-b backdrop-blur-2xl transition-all ${
          isDark 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 cursor-pointer focus:outline-none transition-transform hover:scale-102"
              title="Kembali ke Beranda Chemly"
            >
              <ChemlyLogo size="md" />
            </button>

            <span className={`h-4 w-[1px] ${isDark ? "bg-zinc-800" : "bg-zinc-200"}`} />

            {/* Version Trigger */}
            <button
              type="button"
              onClick={() => setIsChangelogOpen(true)}
              className={`group flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-mono transition-all border cursor-pointer ${
                isDark
                  ? "bg-zinc-900/90 border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-zinc-200"
            ...
            >
              ...
              <span className="opacity-40 group-hover:opacity-100 transition-opacity font-sans">log</span>
            </button>
          </div>
          ...
          >
            ...
            >
              <span className="font-mono text-[10px] text-sky-400 font-bold flex items-center gap-1">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button
              type="button"
              onClick={onOpenLanding}
              className={`hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-colors cursor-pointer ${
                isDark 
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all cursor-pointer ${
                isDark
                  ? "bg-zinc-900/70 hover:bg-zinc-800 border-zinc-800 text-zinc-200 hover:text-white"
                  : "bg-zinc-100/80 hover:bg-zinc-200 border-zinc-300 text-zinc-700 hover:text-black"
              }`}
            >
              <Layers className="w-3.5 h-3.5 text-sky-400" />
              <span className="hidden sm:inline">Tabel Periodik</span>
              <span className="text-[10px] font-mono font-bold px-1.5 py-0.2 rounded border border-sky-500/30 bg-sky-500/10 text-sky-400">
                118
              </span>
            </button>

            {/* Metodologi Trigger Button */}
            <button
              type="button"
              ...
              title="Pelajari arsitektur dan metodologi komputasi Chemly"
            >
            ...
            {onOpenNotes && (
              <button
                type="button"
                onClick={onOpenNotes}
                className={`hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-colors cursor-pointer ${
                  isDark 
                    ? "text-zinc-400 hover:text-white hover:bg-zinc-900" 
                    : "text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100"
                }`}
            ...
            >
              {isDark ? <Sun className="w-3.5 h-3.5 text-amber-400" /> : <Moon className="w-3.5 h-3.5 text-zinc-700" />}
            </button>

            {/* High-Contrast AI Assistant Trigger */}
            <button
              type="button"
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all shadow-md cursor-pointer ${
                isDark
                  ? "bg-zinc-100 hover:bg-white text-zinc-900 font-bold shadow-white/5"
                  : "bg-zinc-900 hover:bg-zinc-800 text-white font-bold shadow-zinc-400/20"
              }`}
            >
