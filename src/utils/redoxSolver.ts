...
function findPositiveNullVector(
  ...
  /*
   * Robust exact null-space search.
   *
   * The previous implementation picked the first free variable (or set all
   * free variables to 1). That is unsafe when auxiliary species (H+, OH-,
   * H2O, e-) are introduced: a higher-dimensional null space can contain
   * several positive solutions, and the first one is not necessarily the
   * primitive/minimal chemical ratio.
   *
   * We now:
   *   1. RREF the exact Rational matrix.
   *   2. Enumerate small positive integer values for the free variables.
   *   3. Construct the exact dependent variables.
   *   4. Reject vectors with negative/zero coefficients.
   *   5. Convert to primitive integers and select the vector with the
   *      smallest coefficient sum (then smallest max coefficient).
   *
   * This keeps the solver deterministic while avoiding floating-point
   * rounding and the erroneous 2:14:16 -> 1:14:8 result.
   */

  const M: Rational[][] = A.map((row) =>
    row.map((x) => new Rational(x.numerator, x.denominator))
  );
  ...
  for (let col = 0; col < numCols && pivotRow < numRows; col++) {
    let row = pivotRow;
    while (row < numRows && M[row][col].isZero()) row++;
    ...
    for (let c = 0; c < numCols; c++) {
      M[pivotRow][c] = M[pivotRow][c].div(pivot);
    }
  ...
  const normalize = (values: Rational[]): number[] | null => {
    if (values.some((v) => !v.isPositive())) return null;

    let lcm = 1n;
    for (const v of values) {
      lcm = Rational.lcm(lcm, v.denominator);
    }

    const ints = values.map((v) =>
      Number((v.numerator * lcm) / v.denominator)
    );

    if (ints.some((v) => !Number.isFinite(v) || v <= 0)) return null;

    let g = BigInt(ints[0]);
    for (let i = 1; i < ints.length; i++) {
      g = Rational.gcd(g, BigInt(ints[i]));
    }
    if (g <= 0n) return null;

    return ints.map((v) => Number(BigInt(v) / g));
  };
  ...
  // Homogeneous systems can be scaled, so searching small positive free
  // values is sufficient for the primitive solution in normal chemical
  // balancing systems. Keep the bound deliberately conservative.
  const SEARCH_MAX = freeCols.length <= 3 ? 24 : 10;

  const evaluate = (freeValues: number[]) => {
    const x: Rational[] = Array.from(
      { length: numCols },
      () => Rational.zero()
    );

    freeCols.forEach((col, i) => {
      x[col] = Rational.fromNumber(freeValues[i]);
    });

    // RREF equation: x[p] + Σ M[row][free] x[free] = 0
    for (let i = pivotCols.length - 1; i >= 0; i--) {
      const pCol = pivotCols[i];
      let sum = Rational.zero();

      for (let c = 0; c < numCols; c++) {
        if (c === pCol || M[i][c].isZero()) continue;
  ...
  // Enumerate all positive free-variable assignments.
  const values = new Array<number>(freeCols.length).fill(1);
